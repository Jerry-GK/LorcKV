...
VecRange::VecRange(VecRange&& other) noexcept {
...
VecRange::VecRange(std::shared_ptr<RangeData> data, int subrange_view_start_pos, size_t size) {
    this->data = data;
...
VecRange& VecRange::operator=(VecRange&& other) noexcept {
    if (this != &other) {
...
std::string& VecRange::startKey() const {
    assert(valid && size > 0 && data->keys.size() > 0 && subrange_view_start_pos == -1);
    return data->keys[0];
}

std::string& VecRange::endKey() const {
    assert(valid && size > 0 && data->keys.size() > 0 && subrange_view_start_pos == -1);
    return data->keys[size - 1];
}   

std::string& VecRange::keyAt(size_t index) const {
    assert(valid && size > index && subrange_view_start_pos == -1);
    return data->keys[index];
}

std::string& VecRange::valueAt(size_t index) const {
    assert(valid && size > index && subrange_view_start_pos == -1);
    return data->values[index];
}

std::vector<std::string>& VecRange::getKeys() const {
    assert(valid && size > 0 && data->keys.size() > 0 && subrange_view_start_pos == -1);
    return data->keys;
}

std::vector<std::string>& VecRange::getValues() const {
    assert(valid && size > 0 && data->values.size() > 0 && subrange_view_start_pos == -1);
    return data->values;
}

size_t VecRange::getSize() const {
    return size;
}   

bool VecRange::isValid() const {
    return valid;
}

int VecRange::getTimestamp() const {
    return timestamp;
}

void VecRange::setTimestamp(int timestamp) const {
    this->timestamp = timestamp;
}

VecRange VecRange::subRangeView(size_t start_index, size_t end_index) const {
    ...
    return VecRange(data, start_index, end_index - start_index + 1);
}

VecRange VecRange::subRange(size_t start_index, size_t end_index) const {
    ...
    return VecRange(std::move(sub_keys), std::move(sub_values), end_index - start_index + 1);
}

VecRange VecRange::subRange(std::string start_key, std::string end_key) const {
    ...
    if (start_index == -1 || end_index == -1) {
        return VecRange(false);
    }

    return this->subRange(start_index, end_index);
}

void VecRange::truncate(int length) const {
    assert(valid && size > 0 && data->keys.size() > 0 && subrange_view_start_pos == -1);
...
int VecRange::find(std::string key) const {
    assert(valid && size > 0 && data->keys.size() > 0 && subrange_view_start_pos == -1);
    if (!valid || size == 0) {
        return -1;
    }
...
std::string VecRange::toString() const {
    std::string str = "< " + this->startKey() + " -> " + this->endKey() + " >";
    return str;
}

VecRange VecRange::concatRanges(std::vector<VecRange>& ranges) {
    if (ranges.empty()) {
        return VecRange(false);
    }
    ...
    for (const auto& VecRange : ranges) {
        auto keys = VecRange.getKeys();
        auto values = VecRange.getValues();
        mergedKeys.insert(mergedKeys.end(), keys.begin(), keys.end());
        mergedValues.insert(mergedValues.end(), values.begin(), values.end());
    }
    
    return VecRange(std::move(mergedKeys), std::move(mergedValues), mergedKeys.size());
}

// Function to combine ordered and non-overlapping ranges with move semantics
VecRange VecRange::concatRangesMoved(std::vector<VecRange>& ranges) {
    if (ranges.empty()) {
        return VecRange(false);
    }
    ...
    // Move data from each VecRange into the new combined VecRange
    for (auto& VecRange : ranges) {
        auto src_data = VecRange.data; 
        int subrange_view_start_pos = VecRange.subrange_view_start_pos;
        int range_start = 0;
        if (subrange_view_start_pos != -1) {
            range_start = subrange_view_start_pos;
        }
        size_t len = VecRange.getSize();
    ...
    return VecRange(new_data, -1, total_size);
}