...
#include "cache/LogicallyOrderedSliceVecRangeCache.h"
...
void emplaceFromRangeCache(SliceVecRange& r, const Slice& key, const Slice& value) {
    // should be locked
    r.emplace(key, value);
}

void emplaceFromKVMap(SliceVecRange& r, const std::string& key, const std::string& value) {
    // should be locked
...
void executeRangeQuery(KVMap* kv_map, LogicallyOrderedSliceVecRangeCache* lorc, const std::string& start_key, int range_len) {    
    Logger::info("Executing SliceVecRange query: < StartKey = " + start_key + ", len = " + std::to_string(range_len) + ", EndKey = " \
        + std::to_string((std::stoi(start_key) + range_len - 1)) + " >");
    SliceVecRange slice_vec_range(true);
    slice_vec_range.reserve(range_len);

    int len = 0;
    SliceVecRangeCacheIterator* rc_it = lorc->newSliceVecRangeCacheIterator();
    ...
    while (len < range_len) {
        if (rc_it->Valid() && (!kv_it->Valid() || rc_it->key() <= kv_it->key())) {
            ...
            while (rc_it->Valid() && len < range_len) {
                ...
                emplaceFromRangeCache(slice_vec_range, rc_it->key(), rc_it->value());
                partial_hit = true;
                len++;
                hit_len++;
                rc_it->Next();
            ...
            if (len < range_len) {
                ...
                if (kv_it->Valid() && kv_it->key() == last_read_rc_key) {
                    kv_it->Next(); // skip the last key read in SliceVecRange cache
                }
            }
        } else if (rc_it->Valid() && (kv_it->Valid() && rc_it->key() > kv_it->key())) {
            emplaceFromKVMap(slice_vec_range, kv_it->key(), kv_it->value());
            len++;
            full_hit = false;
            kv_it->Next();
        } else if (!rc_it->Valid()) {
    ...
    assert(slice_vec_range.getSize() == range_len && range_len == len);
    // if (do_validation) {
    //     bool validation_success = kv_map->Validate(keys, values);
    //     if (!validation_success) {
    //         Logger::error("Validation failed for SliceVecRange: < StartKey = " + start_key + ", len = " + std::to_string(range_len) + ", EndKey = " \
    //             + std::to_string((std::stoi(start_key) + range_len - 1)) + " >");
    //     }
    // }   
...
int main() {
    ...
    LogicallyOrderedSliceVecRangeCache* lorc = new RBTreeSliceVecRangeCache(cache_size);
    ...
    if (lorc->enableStatistic()) {
        ...
        Logger::info("Put SliceVecRange Num: " + std::to_string(lorc->getCacheStatistic().getPutRangeNum()));
        Logger::info("Get SliceVecRange Num: " + std::to_string(lorc->getCacheStatistic().getGetRangeNum()));
        Logger::info("Avg Put SliceVecRange Time: " + std::to_string(lorc->getCacheStatistic().getAvgPutRangeTime()) + " us");
        Logger::info("Avg Get SliceVecRange Time: " + std::to_string(lorc->getCacheStatistic().getAvgGetRangeTime()) + " us");
    }
